...
function removeFirstOccurrences(str, value) {
  ...
  const newstr = `${str.slice(0, index)}${str.slice(index + value.length, str.length)}`;
  return newstr;
}
...
function isPalindrome(str) {
  const normalised = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  return normalised.split('').reverse().join('') === normalised;
}
...
function findLongestWord(sentence) {
  return sentence
    .split(' ')
    .reduce((long, current) => (current.length > long.length ? current : long));
}
